...
def cnn_model(x_img_input ,x_age_input,x_sex_input, n_x , n_y , n_z , prob1, prob2, seed=None ):
    ...
    # conv => n_x*n_y*n_z
    conv1 = tf.layers.conv3d(inputs=x_img_input, filters=32, kernel_size=[3,3,3], padding='same', activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), name='conv1')
    # conv => n_x*n_y*n_z
    conv2 = tf.layers.conv3d(inputs=conv1, filters=32, kernel_size=[3,3,3], padding='same', activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), name='conv2')
    ...
    conv4 = tf.layers.conv3d(inputs=pool3, filters=64, kernel_size=[3,3,3], padding='same', activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), name='conv4')
    # conv => n_x*n_y*n_z/2*2*2
    conv5 = tf.layers.conv3d(inputs=conv4, filters=64, kernel_size=[3,3,3], padding='same', activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), name='conv5')
    # pool => n_x*n_y*n_z/4*4*4
    pool6 = tf.layers.max_pooling3d(inputs=conv5, pool_size=[4, 4, 4], strides=4, name='pool6')

    # conv => n_x*n_y*n_z/4*4*4
    conv7 = tf.layers.conv3d(inputs=pool6, filters=128, kernel_size=[3,3,3], padding='same', activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), name='conv7')
    # conv => n_x*n_y*n_z/4*4*4
    conv8 = tf.layers.conv3d(inputs=conv7, filters=128, kernel_size=[3,3,3], padding='same', activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), name='conv8')
    # pool => n_x*n_y*n_z/8*8*8
    pool9 = tf.layers.max_pooling3d(inputs=conv8, pool_size=[4, 4, 4], strides=4, name='pool9')

    cnn3d_bn = tf.layers.batch_normalization(inputs=pool9, training=True)

    flattening = tf.reshape(cnn3d_bn, [-1, (n_x//32)*(n_y//32)*(n_z//32)*128 ])
    dense1 = tf.layers.dense(inputs=flattening, units=512, activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), bias_regularizer = regularizers.l2(regL), name='dense1')
    # (1-keep_rate) is the probability that the node will be kept
    dropout1 = tf.layers.dropout(inputs=dense1, rate=1-prob1, training=True)
    
    dense2 = tf.layers.dense(inputs=dropout1, units=128, activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), bias_regularizer = regularizers.l2(regL), name='dropout1') + 
    tf.layers.dense(inputs=x_age_input, units=128, activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), bias_regularizer = regularizers.l2(regL), name='Relu_age') + 
    tf.layers.dense(inputs=x_sex_input, units=128, activation=tf.nn.relu, kernel_regularizer = regularizers.l2(regL), bias_regularizer = regularizers.l2(regL), name='Relu_sex') 
...
experiment_name = 'B8_2SimplArch_Aug_0.3AND0.4Drop_3Groups_SxAge_L2Regular0.05_biasReg0.05_convReg0.05'
Num_batches = 204
seed = 0
    
batch_train, batch_dev, batch_test = Split_Train_Dev_Test(Num_batches , seed)

train_neural_network(folder, batch_train , batch_dev, experiment_name , learning_rate = 0.0001, keep_rate1 = 0.15, keep_rate2 = 0.25, epochs = 1000)
